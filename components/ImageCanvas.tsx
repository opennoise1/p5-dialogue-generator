...
const ImageCanvas = ({ portrait }) => {
  const [isBoxLoaded, setBoxLoaded]: [boolean, any] = useState(false);
  ...
  useEffect(() => {
    // preps the canvas by creating a 2D "drawing context" for it
  ...
  const draw = (image, x, y, w, h) => {
    ...
    return isBoxLoaded 
    ? ctx.drawImage(box.current, 240, 295, boxWidth, boxHeight)
    : null;
  }
